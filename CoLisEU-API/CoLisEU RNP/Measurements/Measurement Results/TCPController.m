...
- (void)viewDidLoad {
    ...
    graphTCP = [[NSMutableArray alloc]init];
    plotTCP = [[NSMutableArray alloc]init];
    ...
    [[IPerfController getInstance] startTCP];
}
-(void)initPlotTCP{
    NameTest = @"TCP";
    
    NSArray *tcpdw = [IPerfController getInstance].TcpDown;
    AverageTCPDown = @([[tcpdw valueForKeyPath:@"@sum.floatValue"] floatValue]/5/128);
    NSLog(@"lista %@",tcpdw);
    NSLog(@"Average tcp down %f",[AverageTCPDown floatValue]);
    NSArray *tcpup = [IPerfController getInstance].TcpUp;
    AverageTCPUp = @([[tcpup valueForKeyPath:@"@sum.floatValue"] floatValue]/5/128);
    
    
    
    self.lbDownload.text = [NSString stringWithFormat:@"%.2f %@",[AverageTCPDown floatValue],unit];
    self.lbUpload.text = [NSString stringWithFormat:@"%.2f %@",[AverageTCPUp floatValue],unit];
    Flag = 0;
    
    [[NetworkTestList sharedNetworkList].TCPdownList addObject:tcpdw];
    [[NetworkTestList sharedNetworkList].TCPupList addObject:tcpup];
    
    [self ConfigureTest:@"TCP" testSelected:2 testView:self.TCPView];
}
...
-(void)configurePlots {  // Responsável por plotar o gráfico, ou seja monta o gráfico
    ...
    if([NameTest isEqualToString:@"TCP"]){
        [graphTCP addObject:graph];
        [plotTCP addObject:aaplPlot];
        [plotTCP addObject:googPlot];
...
-(void)ConfigureLegends{
  ...
  /*  if ((int)TestSelected == 2){
        ...
        if((int)majorIncrement < 1){
            ...
            NSLog(@"maior que 1");
        NSLog(@"mino and major %f",(float)minorIncrement);
    }*/
    minorIncrement = majorIncrement = 3;
}
-(void)configureAxes {
    ...
    NSString *formatTest = (int)TestSelected == 0 || (int)TestSelected == 1 ? @"(ms)" : unit;
            ...
            switch ((int)TestSelected) {
                case 2:
                    if ([plot.identifier isEqual:CPDTickerSymbolAAPL] == YES){
                        NSLog(@"nsnumber %@",[NSNumber numberWithFloat:[[IPerfController getInstance].TcpDown[index] floatValue]/128]);
                        return [NSNumber numberWithFloat:[[IPerfController getInstance].TcpDown[index] floatValue]/128];
                    }
                    else if ([plot.identifier isEqual:CPDTickerSymbolGOOG] == YES)
                        return [NSNumber numberWithFloat:[[IPerfController getInstance].TcpUp[index] floatValue]/128];
                    break;
            }
            break;
    }
    ...
    switch (notification) {
        case TCP:
            if(![IPerfController getInstance].error){
                NSThread *t = [[NSThread alloc] initWithTarget:self selector:@selector(initPlotTCP) object:nil];
                [t start];
            }
            break;
        default:
...
-(void)nextTest{
    sleep(1);
    [ScreenLoadController showTestUDP:self.slideOutAnimationEnabled];
}
